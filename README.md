...
覆盖范围：自动批量签到（中转站面板，接口路径探测 + 当日幂等 + 如实报错）、模型列表拉取、对话补全、三种响应形状的解析、流内错误识别、usage 解析与缺失时的估算、费用估算与前缀匹配、日志写入与截断、导入导出往返、失败分类（401/403/404/429/超时/域名不通/跨域）、BaseURL 四种写法容错、三条网络通道各自的正确性、四家官方余额接口、中转站面板四种登录形态（明文 / 加密 / 人机验证 / 非面板地址）、凭据落盘与导出脱敏、以及电脑端与手机端两侧的真实计算样式。
