...
签到只存在于 new-api 系的中转站面板 —— **官方平台（OpenAI / DeepSeek / Anthropic 等）没有这回事**，界面不会给你一个点了没反应的签到按钮：只有存在可签账号时，总览操作条上才会出现「一键签到」。

- **凭据与余额共用一套**：优先用已登录面板的 token，没有就账号密码登录换一个；token 失效会明确说「需要重新登录」。
- **接口路径按候选列表探测**：new-api 用 `POST /api/user/check_in`，部分分站挂在 `/api/user/sign_in` —— 挨个试，全 404 就如实报「该站点没有签到功能」，不会假装签上了。
- **幂等**：同一天已签的账号本地直接跳过（不打扰站点）；绕过本地记录时站点回「已经签到」也按成功算。
- **批量**：总览「一键签到」并发 2 跑完整批，失败的账号不打断整批，原因留在各自账号上；结果一句话汇总「成功 N · 今天已签 N · 失败 N」。
- **边界照旧**：开人机验证的站点登录不了就签不了，明说；没有签到接口的站点明确报 `not_supported`。
